...
tau_z = tinyarray.array(np.kron(s_0,s_z)) 	# kron product is ~ a tensor product.
...
def write_file_and_plot(f,par,Ez_values,energies,parameters,parameters_values):
	""" Writing log to file outfile, containing the system parameters, the EZ_values and the eigenenergies, energies. Plots energies vs. Ez.

		Notes
		-----
		 - naming convention : 		EvsEz_ 	+ "Asym_" if asymmetryc/"Sym_" if symmetric device
		 									+ str(datetime.date.today())
		 									+ the following parameters, in order : par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR
	"""
	if par.asymmetric == True:			# if asymmetric device
		plt.title("$N_x$:%g,$N_x^A$:%g,$N_y$:%g,$\mu$:%g,$\Gamma$:%g,$\phi_L$:%g,$\phi_R$:%g,$a_x$:%.0f,$a_y$:%.0f" %(par.Nx,par.Nxasymmetric,par.Ny,par.mu,par.Gamma,par.phiL,par.phiR,par.ax,par.ay),fontsize=16)
		plt.xlabel("$E_z\, [meV]$",fontsize=16)
		plt.ylabel("$E_n\, [meV]$",fontsize=16)
		f.savefig("EvsEz_Asym_%s_%g_%g_%g_%g_%g_%g_%g_%g_%g_'alphabar'_%g_%g_%g.pdf" %(str(datetime.date.today()),par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR))		
		outfile = open(str("%s_datalog_Asym_EvsEz_%g_%g_%g_%g_%g_%g_%g_'alphabar'_%g_%g_%g.txt" %(str(datetime.date.today()),par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.mu,par.Gamma,par.phiL,par.phiR)),"w")

	else:
		plt.title("$N_x$:%g,$N_y$:%g,$\mu$:%g,$\Gamma$:%g,$\phi_L$:%g,$\phi_R$:%g,$a_x$:%.0f,$a_y$:%.0f" %(par.Nx,par.Ny,par.mu,par.Gamma,par.phiL,par.phiR,par.ax,par.ay),fontsize=16)
		plt.xlabel("$E_z\, [meV]$",fontsize=16)
		plt.ylabel("$E_n\, [meV]$",fontsize=16)
		f.savefig("EvsEz_Sym_%s_%g_%g_%g_%g_%g_%g_%g_%g_%g_'alphabar'_%g_%g_%g.pdf" %(str(datetime.date.today()),par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR))
		outfile = open(str("%s_datalog_Sym_EvsEz_%g_%g_%g_%g_%g_%g_%g_'alphabar'_%g_%g_%g.txt" %(str(datetime.date.today()),par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.mu,par.Gamma,par.phiL,par.phiR)),"w")

	Ez_values_str="np.linspace(%g,%g,%g)" %(Ez_values[0],Ez_values[-1],len(Ez_values))
	outfile.write("Parameters=%s;\nEz_values=...;\nenergies=...: \n%s\n%s\n%s.\n\n" %(parameters,str(parameters_values),str(Ez_values_str),str(energies)))
	outfile.close()

def hopping_lw(site0, site1):
	"""Hopping linewidth between site0 and site1. Uses the fact that to site naming convention, sites above/below each other and next to each other must have the y coupling. Else, they have the x coupling."""
...
def Gamma_phi_fn(site,p):
	"""Calculates the value of Gamma and phi given the input site and the input parameters contained in the Namespace class p."""
	if site.pos[0] <= par.left[-1]:	# site.pos[0] is the x value of the position. ?? Could also implement as heavyside step function, but this should work as it is.
...
def make_system(p):
	"""Function building superconductor for parameters from SimpleNamespace object p."""
	sys = kwant.Builder()

	if len(par.right) == 0:		# if no right superconducting 2D layer, the device is asymmetric.
	...
	for i,part_ in zip(('L','M','R'),(par.left, par.middle, par.right)):
		...
		if len(part_) == 0:	# if asymmetric device such that a part is simply removed, effectively rendering its length=0
			if part_ != par.right:
				print("%s: %s (site %s to %s)" %(str(datetime.datetime.now()),i,part_[0],part_[-1]))
			else:
...
def main(**kwargs):
	...
	fig = kwant.plot(sys,site_symbol='o',hop_color='pink',site_edgecolor='red',site_lw=0.01, hop_lw=hopping_lw)
	...
	for par.Ez in Ez_values: # par.Ez in Ez_values adds/overwrites EZ_values[0] etc. to the par dictionary.

		H = sys.hamiltonian_submatrix(args=[par],sparse=True)

		H = H.tocsc() #'Return a copy of this matrix in Compressed Sparse Column format'

		eigs = scipy.sparse.linalg.eigsh(H,k=50,sigma=0) 	# Find k eigenvalues and eigenvectors of the real symmetric square matrix or complex hermitian matrix A.[...] k: number of eigenvalues/vecs desired.[...]. sigma: Find eigenvalues near sigma using shift-invert mode.
		energies.append(np.sort(eigs[0])) 					# this is the energies of the k first eigenstates. Element eigs[1] are the eigenvectors. After the loop, energies are collected in a nested array, which can be accessed as follows: energies[0:len(Ez_values)][0:k]
...
"""Parameters: Lengths (LX, LY) and Nx specified. Then, calculating ax=ay=a=LX/Nx=LY/Ny."""
const = SimpleNamespace(c=2.99792458e17,m_e=0.5109989461e9,hbar=6.582119514e-13) 	# [c]=nm/s, [m_e]=meV/c^2, [hbar]=meV*s
par = SimpleNamespace(LM=250.,LL=1.e3,LR=1.e3,LY=4.e3,mu=0,alphahbar=1.42e-4*const.c*const.hbar,Gamma=180e-3,phiL=0,phiR=0)
																					# [L]=nm, [mu]=meV, [alpha]=nm*meV, [Gamma]=meV, [phi]=1 
																					# the global parameter class
par.GammaL = par.Gamma; par.GammaR=par.Gamma 	# GammaL/R, the effective superconducting gap, used in gamma_phi_fn(site,p)

counter_Hprint = 0								# defined such that only printing parts of H the first time main() is called.

for par.Nx in [120,260]:
	par.Ny = int(round(float(par.Nx/1.625))) 	#rounding off to nearest int. 1.625 defines the aspect ratio Nx/Ny.
	par.ax = float(par.LM+par.LL+par.LR)/par.Nx
	par.tx = const.hbar**2/(2*0.023*const.m_e/(const.c)**2*par.ax**2) 			# [t]=meV(=(meV*s)^2/(meV/(nm/s)^2*nm^2))
	par.ay = float(par.LY)/par.Ny 
	par.m_star = 0.023*const.m_e/(const.c)**2
	par.ty = const.hbar**2/(2*par.m_star*par.ay**2)
	"""	Making part a seperate Namespace parameter holding class such that not called by hamiltonian_submatrix later;
	...
	par.middle = np.arange(round(float(par.LL)/par.ax),round(float(par.LL+par.LM)/par.ax))
	
	for par.right in (np.arange(round(float(par.LL+par.LM)/par.ax),round(float(par.LL+par.LM+par.LR)/par.ax)),[]):		# NL,NM,NR:lattice numbers, starting from 0 in NL
