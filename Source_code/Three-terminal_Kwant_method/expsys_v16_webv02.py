...
ppar = SimpleNamespace(
    # Category (i) :
	# Two-terminal device
	# N	-	N(1D)		<-- '(xD)': dimension of Hamiltonian (x=1: only hopx, x=2: both hopx and hopy) 
	# 		|			<-- lines: connection to leads
	# 		S
    make_1DNLeft_1DN_2DSMiddle_No_NRight=False,\
	# Three-terminal device (energies below SC gap)
	# N	-	S(1D,Heff)	-	N : 	<-- 'Heff': effective model for proximitized semiconductor (Oreg-Lutchyn)
    make_1D_Heff_LR_leads=True, \
	# Two-terminal device 	
	# N	-	N(1D)
	# 		#####		<-- hashtags: connection to '2D layer', inside the scattering region
	# 		S(2D)
	# 		|
	# 		S(2D)
    make_1DNLeft_1DN_2DS_2DSMiddle_No_NRight=False, \
	# Same as above (duplicate) 		<--- Check[IMHERE]
    make_1D_NLeft_1D_N_2D_S_2D_SMiddle_No_NRight=False, \
	# Two-terminal device (energies below SC gap)
	# N -	N(1D)
	# 		#####
	# 		S(1D,Heff)
	...
	# SC lead attached(?) : 
    SClead=False, \
	# SC Pincher layer added in scattering region(?) : 
	pincher_SC_lead=False, \
	# 1D SC pincher <--- impossible to add, anyways (superflous variable[IMHERE]): 
	SC_pincher_1D=False, \
    # Category (iii) :
    generate_En_Ez=None, \
    generate_En_mu=None, \
