...
def set_filename_skeleton(par, ppar):
	...
	if par.pincher == True:
		if ppar.doubleres == False:
			ppar.filename = str.format("Nx%g_Ny%g_LM%g_LL%g_LR%g_LY%g_ax%g_ay%g_mu%g_Gamma%g_pL%g_pR%g_k%g_biE%g_%g_Ez%g_%g_mu%g_%g_biEres%g_Ezres%g_mures%g_pinchcoef%g_tsys%g_tLeads%g_" %(par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR,par.k,par.biasenergies[0],par.biasenergies[-1],par.Ez_values[0],par.Ez_values[-1],par.mu_values[0],par.mu_values[-1],par.biasEres,par.Ezres,par.mures,par.t_p_coeff,par.ty,par.t_N_lead))	# name is for both fig and datalog files and specifies only the parameters such that can be used for any general file if needed; adding file extension later when saving the respective files
		if ppar.doubleres == True:
			...
			ppar.filename = str.format("Nx%g_Ny%g_LM%g_LL%g_LR%g_LY%g_ax%g_ay%g_mu%g_Gamma%g_pL%g_pR%g_k%g_biE%g_%g_Ez%g_%g_mu%g_%g_biEres%g_Ezres%g_mures%g_pinchcoef%g_tsys%g_tLeads%g_" %(par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR,par.k,par.biasenergies[0],par.biasenergies[-1],par.Ez_values[0],par.Ez_values[-1],par.mu_values[0],par.mu_values[-1],par.biasEres,par.Ezres,par.mures,par.t_p_coeff,par.ty,par.t_N_lead))	
			ppar.filename_2 = str.format("Nx%g_Ny%g_LM%g_LL%g_LR%g_LY%g_ax%g_ay%g_mu%g_Gamma%g_pL%g_pR%g_k%g_biE%g_%g_Ez%g_%g_mu%g_%g_biEres%g_Ezres%g_mures%g_pinchcoef%g_tsys%g_tLeads%g_" %(par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR,par.k,par.biasenergies_2[0],par.biasenergies_2[-1],par.Ez_values_2[0],par.Ez_values_2[-1],par.mu_values_2[0],par.mu_values_2[-1],par.biasEres_2,par.Ezres_2,par.mures_2,par.t_p_coeff,par.ty,par.t_N_lead))
	else:
		if ppar.doubleres == False:
			ppar.filename = str.format("Nx%g_Ny%g_LM%g_LL%g_LR%g_LY%g_ax%g_ay%g_mu%g_Gamma%g_pL%g_pR%g_k%g_biE%g_%g_Ez%g_%g_mu%g_%g_biEres%g_Ezres%g_mures%g_ty%g_tLeads%g_" %(par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR,par.k,par.biasenergies[0],par.biasenergies[-1],par.Ez_values[0],par.Ez_values[-1],par.mu_values[0],par.mu_values[-1],par.biasEres,par.Ezres,par.mures,par.ty,par.t_N_lead))
		if ppar.doubleres == True:
			### Insert filename_2, filename as two different filenames here???
			ppar.filename = str.format("Nx%g_Ny%g_LM%g_LL%g_LR%g_LY%g_ax%g_ay%g_mu%g_Gamma%g_pL%g_pR%g_k%g_biE%g_%g_Ez%g_%g_mu%g_%g_biEres%g_Ezres%g_mures%g_ty%g_tLeads%g_" %(par.Nx,par.Ny,par.LM,par.LL,par.LR,par.LY,par.ax,par.ay,par.mu,par.Gamma,par.phiL,par.phiR,par.k,par.biasenergies_2[0],par.biasenergies_2[-1],par.Ez_values_2[0],par.Ez_values_2[-1],par.mu_values_2[0],par.mu_values_2[-1],par.biasEres_2,par.Ezres_2,par.mures_2,par.ty,par.t_N_lead))

	if ppar.make_1DNLeft_1DN_2DS_2DSMiddle_No_NRight:
		ppar.filename = "NS_wire_" + ppar.filename 

	if ppar.make_1DNLeft_1DN_2DSMiddle_No_NRight:
		ppar.filename = "N_wire_" + ppar.filename

	if ppar.SClead:
		if par.tx_SC_lead:
			ppar.filename = "txySClead_%.1f_%.1f_tNlead_%.1fmeV_gSgN_%g_" %(par.tx_SC_lead,par.ty_SC_lead,par.t_N_lead,par.g_S_over_g_N) + ppar.filename
		if hasattr(par,"tx_SC_pincher"):
			ppar.filename = "txySCp_%.1f_%.1f" %(par.tx_SC_pincher,par.ty_SC_pincher) + ppar.filename
		else:
	...
	# if ppar.doubleres == False:!!!FIX (see below)
	...
	#!!! FIX: never filename_2/res2 for the conductances, only for the energies/u2mv2	
